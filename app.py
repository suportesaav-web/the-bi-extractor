...
def main() -> None:
    ...
    if is_powerbi:
        ...
        if search_query:
            df_filtered = df_filtered[
                df_filtered["Portfolio"].str.lower().str.contains(search_query, regex=False)
                | df_filtered["Business Unit"].str.lower().str.contains(search_query, regex=False)
                | df_filtered["Customer Group"].str.lower().str.contains(search_query, regex=False)
            ]
        ...
        if view_mode == "📋 Dados Normalizados (Tidy Data)":
            ...
            df_display = build_dataframe_with_total(df_filtered) if include_total_in_table else df_filtered.copy()
            if not df_display.empty:
                df_display = df_display.copy()
                df_display["% PPs"] = df_display["% PPs"] * 100.0
            ...
            with col_exp2:
                ...
                csv_bytes = df_filtered.to_csv(index=False, sep=";").encode("utf-8-sig")
                st.download_button(
    ...
    # ---------------------------------------------------------
        ...
        df_filtered = df_raw_tidy.copy()
        if search_gen:
            mask = df_filtered.astype(str).apply(lambda col: col.str.lower().str.contains(search_gen, na=False, regex=False)).any(axis=1)
        ...
        with col_g2:
            ...
            st.caption("CSV padronizado com delimitador ';' e codificação UTF-8 com BOM.")
            csv_bytes = df_filtered.to_csv(index=False, sep=";").encode("utf-8-sig")
            st.download_button(
